...
<?php
    ...
    {
        $did = $_GET['did'];
        $deleteq = mysqli_query($connection,"Delete From order_tbl where order_no='{$did}'")or die(mysqli_error($connection));
    ...
    //$selectq = mysqli_query($connection, "Select * From order_tbl")or die(mysqli_error($connection));

    $sql = "SELECT
    `order_tbl`.`order_no`
    , `order_tbl`.`order_type`
    , `order_tbl`.`order_details`
...
FROM
    ...
    INNER JOIN `order_tbl` 
        ON (`menu_tbl`.`menu_id` = `order_tbl`.`menu_id`)
    INNER JOIN `customer_tbl` 
        ON (`customer_tbl`.`cust_id` = `order_tbl`.`cust_id`);";
    ...
    while($row = mysqli_fetch_array($query))
    {
        ...
        <a href='Table_Order.php?did={$row['order_no']}' class='btn btn-danger shadow btn-xs sharp'><i class='fa fa-trash'></i></a></td>";
        echo "</tr>";
    }
    ...
    {
        echo "<tr>";
        echo "<td>{$orderrow['order_no']}</td>";
        echo "<td>{$orderrow['order_type']}</td>";
        echo "<td>{$orderrow['order_details']}</td>";
        echo "<td>{$orderrow['order_date']}</td>";
        echo "<td>{$orderrow['menu_id']}</td>";
        echo "<td>{$orderrow['cust_id']}</td>";
        echo "<td>
        <a href='#' class='btn btn-primary shadow btn-xs sharp me-1'><i class='fas fa-pencil-alt'></i></a>
        <a href='Table_Order.php?did={$orderrow['order_no']}' class='btn btn-danger shadow btn-xs sharp'><i class='fa fa-trash'></i></a></td>";
        echo "</tr>";
    }*/
?>
                                        </tbody>
