...
<?php
    ...
    {
        $did = $_GET['did'];
        $deleteq = mysqli_query($connection,"Delete From payment_tbl where payment_id='{$did}'")or die(mysqli_error($connection));
    ...
    , `payment_tbl`.`payment_details`
    , `payment_tbl`.`payment_status`
    , `customer_tbl`.`cust_fname`
    , `booking_tbl`.`booking_details`
    , `order_tbl`.`order_type`
FROM
    `booking_tbl`
    INNER JOIN `payment_tbl` 
        ON (`booking_tbl`.`booking_id` = `payment_tbl`.`booking_id`)
    INNER JOIN `order_tbl` 
        ON (`order_tbl`.`order_no` = `payment_tbl`.`order_no`)
    INNER JOIN `customer_tbl` 
        ON (`customer_tbl`.`cust_id` = `payment_tbl`.`cust_id`);";
    ...
    while($row = mysqli_fetch_array($query))
    {
        ...
        <a href='Table_Payment.php?did={$row['payment_id']}' class='btn btn-danger shadow btn-xs sharp'><i class='fa fa-trash'></i></a></td>";
        echo "</tr>";
    }
    ...
    /*while($paymentrow = mysqli_fetch_array($selectq))
    {
        ...
        echo "<td>{$paymentrow['payment_mode']}</td>";
        echo "<td>{$paymentrow['payment_date']}</td>";
        echo "<td>{$paymentrow['payment_details']}</td>";
        echo "<td>{$paymentrow['payment_status']}</td>";
        echo "<td>{$paymentrow['cust_id']}</td>";
        echo "<td>{$paymentrow['booking_id']}</td>";
        echo "<td>{$paymentrow['order_no']}</td>";
        echo "<td>
        <a href='#' class='btn btn-primary shadow btn-xs sharp me-1'><i class='fas fa-pencil-alt'></i></a>
        <a href='Table_Payment.php?did={$paymentrow['payment_id']}' class='btn btn-danger shadow btn-xs sharp'><i class='fa fa-trash'></i></a></td>";
        echo "</tr>";
    }*/
?>
                                        </tbody>
